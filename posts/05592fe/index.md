...
**内存布局分析：**
  ...
  * **单个节点大小**：这个 Bomb Lab 节点由两个 `int` 和一个指针组成，在常见 x86-64 环境中占 16 字节。其他程序的结构体字段和对齐方式可能不同，应以调试信息或实际地址间隔为准。
...
**指令**：`x/2xg 0x6032d0`
  ...
  * **x (十六进制)**：地址通常以 Hex 显示。
  * **2**：查看当前节点的两个 8 字节块；第二个块对应 `next`。如果要沿链表继续看，应取出 `next` 后再检查它指向的节点。
...
> 作者: 7M7  
> URL: https://7m7666.github.io/posts/05592fe/  
