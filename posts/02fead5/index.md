# CSAPP Attack Lab：栈溢出与 ROP 实验笔记


记录 CSAPP Attack Lab 的栈溢出、代码注入和 ROP 解题思路，整理实验过程里最容易混淆的地址、传参和栈布局问题。

<!--more-->

作为计算机系统基础的核心实验与期末考试高频考点，CS:APP 的 Attack Lab 让我彻底跳出高级语言抽象层，直面 x86-64 架构底层逻辑。从简单栈溢出跳转，到应对地址随机化的 ROP 高级攻击，五个阶段的攻关不仅锤炼了硬核技能，也让我开始从机器执行过程理解代码。本文把实战思路、踩坑经历与复习知识点放在一起，既记录攻击实现细节，也保留我当时的排查过程。

> 实验地址、Cookie 和 Gadget 都与具体 target 绑定。文中的数值只能解释构造方法，复现时必须使用自己的 `ctarget`、`rtarget` 和反汇编结果。
...
1. **x86-64 ABI核心规则（简答题）**
   ...
   - 栈对齐要求：按 System V AMD64 ABI，在执行 `call` 前栈指针应按 16 字节对齐；`call` 压入 8 字节返回地址，因此函数入口通常满足 `%rsp mod 16 == 8`。
     - ROP 链没有正常的 `call`，必须把每个 `ret`、`pop` 和栈上参数占用的 8 字节都算进去，再在目标函数入口检查实际 `%rsp`。
     - 实验错误分析：初始方案在这次 target 上进入 `touch2` 时发生崩溃，最终通过调整跳转方式和栈布局解决；是否属于对齐问题，应以断点处的 `%rsp` 和崩溃指令为准。
...
> 作者: 7M7  
> URL: https://7m7666.github.io/posts/02fead5/  
